...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then((res) => {
          try {
            // 修复zzzb的jbqjs错误
            if (
              res.data.data.jbqjsnm != undefined &&
              res.data.data.jbqjsnm.length > 0
            ) {
              res.data.data.jbqjsnm.forEach((item) => {
                item.jbqjsnm = item.jbysnm;
                delete item.jbysnm;
              });
            }
    ...
    handleGetZbList() {
      this.$http
        .get(this.$api[this.zbkey].url)
        .then((res) => {
          this.zbList = res.data.data;
          sessionStorage.setItem("_zzzb_zb", JSON.stringify(res.data.data));
        })
        .catch((err) => {
          console.log(err);
        });
  ...
  created() {
    ...
    this.handleGetZbList();

    this.getZbSelect().then((res) => {
      this.zbSelect = res;
      console.log(res);
    });
  },
