  ...
  <div class="wrapper">
    ...
    >
      <el-tab-pane
        ...
        "
      >
        <div v-if="Array.isArray(item)" name="table">
          <tab-inner-table
            :ref="`table${index}`"
            :item="item"
            :showIndex="true"
                ...
                <div v-if="isSelect(selects, item2.field)">
                  ...
                  >
                    <el-cascader
                      ...
                      v-model="dataCopy.tables[index - 1][item2.field]"
                      :show-all-levels="false"
                  ...
                  <div v-if="item2.field !== 'bdnm'">
                    ...
                    >
                      <el-date-picker
                        ...
                        @change="
                          handleDateFormat(
                            dataCopy.tables[index - 1][item2.field],
                            item2.field,
                            index
                          )
                        "
...
export default {
  ...
  methods: {
    ...
    handleDateFormat(value, field, index) {
      if (index != undefined) {
        this.dataCopy.tables[index - 1][field] = new Date(
          String(value)
        ).getTime();
      } else {
        ...
        this.data.tables.forEach((item, index) => {
          ...
          if (this.isObject(item)) {
            item.fields.forEach((item2, index2) => {
              for (let i in this.dataCopy.tables[index - 1]) {
                if (item2.require === "1") {
                  let _arr = [];
                  _arr.push({
                    required: true,
    ...
    handleOnSubmit(nm) {
        ...
        this.data.tables.forEach((item, index) => {
          ...
          if (this.isObject(item)) {
            item.fields.forEach((item2, index2) => {
              for (let i in this.dataCopy.tables[index - 1]) {
                if (item2.field === i) {
                  item2.fieldValue = this.dataCopy.tables[index - 1][i];
                }
              }
            });
          }
      ...
      for (let i in this.$refs) {
        if (i.includes("table")) {
          const t_index = Number(i.replace(/[^0-9]/gi, ""));
          let _arr = [];
          this.$refs[i][0].tableData.forEach((item) => {
            let _item = JSON.parse(
              JSON.stringify(this.data.tables[t_index][0])
            );
            _item.fields.forEach((item2) => {
              for (let j in item) {
                if (item2.field === j) {
                  item2.fieldValue = item[j];
                }
              }
            });
            _arr.push(_item);
          });
          this.data.tables[t_index] = _arr;
        }
      }
    ...
    handleSetDefaultTab() {
      if (this.data.tables && this.data.tables.length > 0) {
        this.activeTabName = this.data.tables[0][0].tableName;
      }
    },
